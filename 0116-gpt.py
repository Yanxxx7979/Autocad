...
import math
import json
from pictool import calculate_x_coordinate,calculate_y_coordinate,process_dwg_within_area
# OpenAI API Key
  ...
  "messages": [
    {
      ...
      "content": [
        {
          "type": "text",
            "text": "這是一個插頭的以正投影第三角投影法做出的視圖圖檔，"
            "圖中包含上下前後左右視圖，視圖通常以十字排列，中間是前視圖、右邊是右視圖、下方是下視圖、左邊是左視圖，"
            "通常左前右視圖會水平繪製、上前下視圖會垂直放置。圖片中已經事先用OD框選出crop，請幫我找內容含有{{keyword}}"
            "是哪個crop，以json格式將座標＋此視圖應放置在{{position}}位置"
            "，依範例格式{'xmin': , 'ymin': , 'xmax': , 'ymax': , 'position': } 回傳，請注意回傳時僅須回傳json內容，不需任何其餘說明。"
